...
RType::Server::Room::Room(std::string const &name, std::shared_ptr<RType::Server::Network::IUdpServer> udpSocket, std::shared_ptr<RType::Server::User> owner, uint32_t id) noexcept :
    users({ owner }), _name(name), _udpSocket(udpSocket), _id(id), _clock(2), _gameObjectId(0)
{
    _clock.setNextMoment(std::vector<int>{60, 10});
}
...
void RType::Server::Room::update() noexcept
{
    if (_clock.numberOfRep(0))
        for (auto &player: _players) {
            ...
            sendRequestToUsers(RType::Network::Request<RType::Network::Udp::PlayerUpdate>(RType::Network::Udp::ActionCodes::SendPlayerUpdate, RType::Network::Udp::PlayerUpdate(position, acceleration, player->getHealth(), player->getScore(), static_cast<RType::Network::Udp::PlayerID>(player->getId()), player->id)));
        }

//    if (_clock.numberOfRep(1))
//        for (auto gameobject : _gameObjects) {
//            gameobject->update(RType::AEvent(), _gameObjects, this);
//        }
//
//    for (uint16_t nbMonsterToGenerate = _clock.numberOfRep(1); nbMonsterToGenerate > 0; --nbMonsterToGenerate) {
//        ++_gameObjectId;
